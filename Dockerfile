...
ENV PATH="/sage:${HOME}/.local/bin:${PATH}"
...
RUN /sage/sage -python -m pip install --user --no-cache-dir -e ".[dev]" && \
    mkdir -p $(/sage/venv/bin/jupyter --data-dir)/kernels && \
    ln -sf /sage/venv/share/jupyter/kernels/sagemath \
      $(/sage/venv/bin/jupyter --data-dir)/kernels/sagemath
